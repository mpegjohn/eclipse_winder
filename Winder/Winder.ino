...
void requestEvent() {
	...
	if (current_mode == testMode) {
		...
		uint8_t status_data[I2C_TX_LENGTH];
		...
		*status_data_pointer++ = this_layer;
		...
		status_data_pointer = doubleToData(current_turns.bytes,
				status_data_pointer);
		status_data_pointer = doubleToData(current_layer_turns.bytes,
				status_data_pointer);
		status_data_pointer = doubleToData(current_speed.bytes,
				status_data_pointer);
		status_data[I2C_TX_LENGTH - 2] = direction;
		status_data[I2C_TX_LENGTH - 1] = running;

		Wire.write(status_data, I2C_TX_LENGTH);
...
void receiveEvent(int howMany) {
	...
	if (command == 0x00) // I2C test
		...
		uint8_t parameters[I2C_RX_LENGTH];
		...
		parameters_pointer = get_float_from_array(wire_size.bytes,
				parameters_pointer);
		parameters_pointer = get_float_from_array(turns.bytes,
				parameters_pointer);
		parameters_pointer = get_float_from_array(spool_length.bytes,
				parameters_pointer);
		parameters_pointer = get_float_from_array(turns_per_layer.bytes,
				parameters_pointer);

		num_layers = *parameters_pointer++;

		parameters_pointer = get_float_from_array(last_layer_turns.bytes,
				parameters_pointer);
...
float calculateSpoolSpeed() {
	...
	float spoolSpeed = ((analog_value / 1023.0) * (MAX_SPEED - MIN_SPEED))
			+ MIN_SPEED;
	return spoolSpeed;
}
